...
const PostuladosDetail: React.FC<PostuladosDetailProps> = ({ item }) => {
  ...
  const renderComponent = () => {
    switch (options[currentIndex]) {
        ...
        return <div>
          {item.education.map((edu) => (
            <div className="text-left flex pb-4 pt-4 bg-gray-300 rounded-t-xl mb-2">
              <div><h1 className="px-4 bg-white m-2 font-bold text-base leading-5 text-purple-900 rounded-lg">{edu.fechaInicio.split("/")[1]} / {edu.fechaFin.split("/")[1]}</h1></div>
              <div><h1 className="font-bold text-base leading-5 text-purple-900">{edu.titulo}</h1>
                <h1 className="font-bold text-xs leading-4 text-gray-600">{edu.institucion}</h1>
                <h1 className="font-semibold text-xs leading-4 text-gray-400">{edu.fechaInicio} - {edu.fechaFin} </h1>
      ...
      case "+Skills":
        return (<div className="text-left p-4 bg-gray-300 rounded-t-xl mb-2 font-bold text-base leading-5 text-purple-900">
        <h1>Idiomas</h1>
  ...
  return (
    <div className="font-inter items-center text-center rounded-xl xl:w-1/4 lg:w-1/3 md:w-1/2 m-2 shadow-md">
      <div className="bg-gray-100 bg-opacity-30 rounded-tl-xl rounded-tr-xl pb-4">
        ...
        <p className=" font-normal text-gray-500 text-base leading-5 mx-2">
          {item.cargo}
        </p>
      </div>
      <div className="flex p-2">
        <div className="flex flex-col p-2">
          <p className="w-full font-semibold text-xs leading-4 text-purple-900">
            Modalidad
          </p>
      ...
      {isOpen && (
        <div ref={popupRef} className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 ">
          <div className="bg-white p-4 rounded-xl w-1/3">
            <h2 className="font-bold text-2xl leading-10 text-violet-800">
              {item.cargo}
            </h2>
            <div className="bg-white border-4 border-gray-100 rounded-xl p-4 flex w-full">
              ...
              <div className="m-2 text-left w-1/3">
                <p className="font-bold text-lg leading-6 text-purple-900">
                  {item.nombre}
                </p>
              ...
              <div className="m-2 w-1/3">
                <div className="flex w-full ">
                  <div className="w-1/3 bg-white border border-purple-900 rounded-md m-1 py-1"><i className="fa-brands fa-linkedin fa-lg"></i></div>
                  <div className="w-1/3 bg-white border border-purple-900 rounded-md m-1 py-1"><i className="fa-brands fa-github fa-lg"></i></div>
                  <div className="w-1/3 bg-white border border-purple-900 rounded-md m-1 py-1"><i className="fa-sharp fa-solid fa-download fa-lg"></i></div>
                </div>
              </div>
            </div>
            <div className="flex bg-gray-300 rounded-xl mt-4 mb-4 font-semibold text-base leading-5 text-purple-900 w-full">
              {options.map((option, index) => (
                <button
                  ...
                  className={`p-4 rounded-xl w-1/4 ${currentIndex === index
